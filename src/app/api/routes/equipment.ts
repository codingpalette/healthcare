...
equipmentRoutes.get("/:id", async (c) => {
  ...
  if (error) return c.json({ error: error.message }, 400)
  if (!data) return c.json({ error: "기구를 찾을 수 없습니다" }, 404)
  return c.json(data)
})
...
equipmentRoutes.post("/", async (c) => {
  ...
  if (userRole !== "trainer") {
    return c.json({ error: "트레이너만 기구를 등록할 수 있습니다" }, 403)
  }
  ...
  if (!name?.trim()) {
    return c.json({ error: "기구명을 입력해주세요" }, 400)
  }
  if (!category) {
...
equipmentRoutes.patch("/:id", async (c) => {
  const userRole = c.get("userRole")
  if (userRole !== "trainer") {
    return c.json({ error: "트레이너만 기구 정보를 수정할 수 있습니다" }, 403)
  }
  ...
  if (!existing) return c.json({ error: "기구를 찾을 수 없습니다" }, 404)
...
equipmentRoutes.delete("/:id", async (c) => {
  const userRole = c.get("userRole")
  if (userRole !== "trainer") {
    return c.json({ error: "트레이너만 기구를 삭제할 수 있습니다" }, 403)
  }
